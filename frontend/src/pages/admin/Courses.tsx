...
export default function AdminCourses() {
  ...
  return (
    <div className="space-y-5">
      ...
      {showManual && (
        <div className="card p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-stone-700">Add Course Manually</h2>
            <button onClick={() => setShowManual(false)} className="text-stone-300 hover:text-stone-500 p-1"><X size={18} /></button>
          </div>
      ...
      {/* Search */}
      <div className="card p-4 sm:p-5">
        <h2 className="font-semibold text-stone-700 mb-3">Search &amp; Import</h2>
        ...
        {results.length > 0 && (
          <div className="mt-3 border border-stone-200 rounded-lg divide-y divide-stone-100 max-h-72 overflow-y-auto">
            {results.slice(0, 20).map((c, i) => (
              <div key={i} className="flex items-center justify-between px-3 py-2.5 gap-3">
                <div className="min-w-0">
                  <div className="font-medium text-sm text-stone-800 truncate">{c.club_name ?? c.course_name}</div>
      ...
      <div>
        ...
        {courses.length === 0 ? (
          <div className="card p-8 text-center text-stone-400 text-sm">No courses saved. Search above to import.</div>
        ) : (
          <div className="card divide-y divide-stone-100">
            {courses.map(course => (
              <div key={course.id}>
                ...
                >
                  <div className="min-w-0">
                    <div className="font-medium text-stone-800 truncate">{course.name}</div>
                    {(course.city || course.state) && (
                      <div className="text-xs text-stone-400 mt-0.5">{[course.city, course.state].filter(Boolean).join(', ')}</div>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-2 text-xs text-stone-400">
                  ...
                  <div className="bg-stone-50 border-t border-stone-100 px-4 py-4">
                    ...
                    <div className="space-y-2">
                      {course.tees.map((tee, i) => (
                        <div key={i} className="bg-white rounded-lg border border-stone-200 px-3 py-2.5 flex items-center justify-between gap-2 flex-wrap text-sm">
                          <span className="font-medium text-stone-700">{tee.name}</span>
