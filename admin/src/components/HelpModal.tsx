...
export function HelpStep({ n, children }: { n: number; children: React.ReactNode }) {
  return (
    <div className="flex gap-2.5">
      <span className="flex-shrink-0 w-5 h-5 rounded-full bg-ep-green/10 text-ep-green text-xs font-bold flex items-center justify-center mt-0.5">
        {n}
      </span>
...
export function HelpTip({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-ep-sand border border-ep-silver/30 rounded-lg px-3 py-2 text-xs text-ep-green/80 leading-relaxed">
      {children}
    </div>
  );
}
